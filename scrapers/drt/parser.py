"""
DRT Parser — transforms raw DRAT API responses into normalized rows.
"""
...
logger = logging.getLogger("legal_scraper.drt.parser")


def _safe_strip(value: Any, suffix: str = "") -> str:
    """Safely strip and clean a string value."""
    if value is None:
        return ""
...
class DRTParser(BaseParser):
    """Parses DRT API responses into normalized output rows."""
    ...
    def build_row(
        ...
        **extra: Any,
        ...
        """Build a normalized row from detail (preferred) or fallback data."""
        d = dict(fallback)
        if detail:
