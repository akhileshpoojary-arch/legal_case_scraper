...
logger = logging.getLogger("legal_scraper.daily_run.hc.parser")

_HC_DETAIL_BASE = "https://hcservices.ecourts.gov.in/hcservices/cases_qry/"
_HC_BASE = "https://hcservices.ecourts.gov.in/hcservices/"
...
def _normalize_hc_order_url(href: str) -> str:
    if not href:
        return ""
    h = href.strip()
    if h.startswith("http://") or h.startswith("https://"):
        return h.replace("/cases_qry/cases/", "/cases/")
    h = h.lstrip("/")
    h = h.replace("cases_qry/cases/", "cases/")
    h = h.replace("cases_qry/", "")
    return _HC_BASE + h
    ...
    if order_table:
        for row in order_table.find_all("tr"):
            ...
            if raw_date and link:
                href = _normalize_hc_order_url(link["href"])
                if href:
...
def _extract_hc_case_number(raw: str) -> str:
    """
    Normalize HC case number by extracting the middle segment from:
    PREFIX/NUMBER/YEAR -> NUMBER
    """
    if not raw:
        return ""
    compact = re.sub(r"\s+", "", str(raw))
    m = re.match(r"^[^/]+/(\d{1,10})/\d{4}$", compact)
    if m:
        return m.group(1)
    m2 = re.match(r"^(\d{1,10})/\d{4}$", compact)
    if m2:
        return m2.group(1)
    return compact
...
def build_hc_row(
    ...
    case_number = _extract_hc_case_number(case_number)

    hc_name = court.get("name", "")
    bench_name = court.get("bench_name", "")
    bench_val = f"{hc_name} (bench: {bench_name})" if hc_name else bench_name

    selected_case_type = _g(court, "selected_case_type")
    final_case_type = selected_case_type or _g(search, "type_name")

    cino = _g(search, "cino", "CINO")
    case_file = f"HC|{cino}" if cino else f"HC|{case_number}|{bench_val}|{registration_date}"
